...
from src.sttcp import general, print_sync

client = Client('c4.play2go.cloud', 20060)
...
def client_response_handler(address: tuple, connection: socket.socket, data: bytes):
    ...
    connection.sendall(str(int(data.decode('utf-8')) + 1).encode('utf-8'))
...
#         print_sync(f'Connection from {":".join(map(str, address))} declined!')
#         return False  # Declining client (also allowed to use connection.close())
#     print_sync(f'Connected by {":".join(map(str, address))}')
...
#     if reason is None:
#         print_sync(f'Disconnected by {":".join(map(str, address))}')
#     else:
#         print_sync(f'Disconnected by {":".join(map(str, address))} with reason {repr(reason)}')
#
#
...
#     print_sync('- Server', handler_type, sep=': ')
...
# @client.connection_handler
# def client_connection_handler(address: tuple, connection: socket.socket):
#     print_sync(f'Connected to {":".join(map(str, address))}')
...
#     print_sync(f'Received string "{response_string}"')
...
#     print_sync(f'Disconnected from {":".join(map(str, address))}')
#
#
# # Defining exceptions handling behavior
# @client.unconnected_handler
# def client_unconnected_handler(address: tuple, exception: Exception):
#     print_sync(f'Could not connect to {":".join(map(str, address))}')
#     raise exception
#
#
# # Defining universal behavior
